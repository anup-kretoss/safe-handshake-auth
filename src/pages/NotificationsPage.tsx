...
import { ArrowLeft, Bell, CheckCircle, AlertCircle, Clock, ShoppingBag, Truck, DollarSign, MessageSquare } from 'lucide-react';
...
const getNotificationIcon = (type: string) => {
  switch (type) {
    case 'item_sold': return <DollarSign className="h-5 w-5 text-green-600" />;
    case 'order_shipped': return <Truck className="h-5 w-5 text-blue-600" />;
    case 'order_delivered': return <CheckCircle className="h-5 w-5 text-green-600" />;
    case 'order_cancelled': return <AlertCircle className="h-5 w-5 text-red-600" />;
    case 'delivery_request':
    case 'seller_delivery_request': return <Clock className="h-5 w-5 text-orange-600" />;
    case 'delivery_approved': return <CheckCircle className="h-5 w-5 text-green-600" />;
    case 'delivery_declined':
    case 'delivery_expired': return <AlertCircle className="h-5 w-5 text-red-600" />;
    case 'new_message': return <MessageSquare className="h-5 w-5 text-blue-600" />;
    case 'new_offer': return <ShoppingBag className="h-5 w-5 text-purple-600" />;
...
const getNotificationColor = (type: string) => {
  switch (type) {
    ...
    case 'order_delivered': return 'border-l-green-500 bg-green-50';
    case 'delivery_request':
    case 'seller_delivery_request': return 'border-l-orange-500 bg-orange-50';
    case 'delivery_declined':
    case 'delivery_expired':
    case 'order_cancelled': return 'border-l-red-500 bg-red-50';
    case 'new_message':
    case 'order_shipped': return 'border-l-blue-500 bg-blue-50';
    case 'new_offer': return 'border-l-purple-500 bg-purple-50';
...
export default function NotificationsPage() {
  ...
  const { data: notifications = [], isLoading, error: queryError } = useNotifications(50);
          ...
          <div className="space-y-3">
            {notifications.map((notif: any) => (
              <div
                ...
                className={`p-4 rounded-2xl border-l-4 border cursor-pointer ${getNotificationColor(notif.type)} transition-all duration-300 ${notif.is_read ? 'opacity-70' : 'shadow-md ring-1 ring-black/[0.02]'}`}
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {notif.data && Object.keys(notif.data).length > 0 && (
                      <div className="flex gap-2 flex-wrap mt-1">
                        ...
                        {(notif.data.delivery_request_id || notif.type === 'delivery_request' || notif.type === 'seller_delivery_request') && (
                          <Link
                            to="/orders"
                            onClick={e => e.stopPropagation()}
